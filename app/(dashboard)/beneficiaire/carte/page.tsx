...
function CarteClient() {
  ...
  const { data } = useRequests({ per_page: 20 })
    ...
    <div className="space-y-4 h-full">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Artisans disponibles près de vous — Conakry</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 h-[calc(100vh-12rem)]">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden relative">
          {mounted ? <LeafletMap artisans={ARTISANS} /> : (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
          <div className="px-4 py-3 border-b border-gray-100">
            <p className="font-semibold text-sm text-gray-900">Artisans proches</p>
            <p className="text-xs text-gray-400">Dans un rayon de 5 km</p>
          </div>
              ...
              <div key={a.id} className="p-4 hover:bg-gray-50 transition-colors cursor-pointer">
                <div className="flex items-center gap-3">
                  <div className="relative flex-shrink-0">
                    ...
                    <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white ${a.available ? 'bg-green-500' : 'bg-gray-400'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 truncate">{a.name}</p>
                    <p className="text-xs text-gray-400">{a.service}</p>
                    <div className="flex items-center gap-1 mt-0.5">
                      <i className="ti ti-star-filled text-amber-400 text-xs" aria-hidden />
...
function LeafletMap({ artisans }: { artisans: any[] }) {
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      map = L.map('leaflet-map', { zoomControl: true }).setView([9.6412, -13.5784], 13)
      ...
      const userIcon = L.divIcon({
        html: `<div style="width:14px;height:14px;border-radius:50%;background:#2460B0;border:3px solid white;box-shadow:0 0 0 4px rgba(36,96,176,0.25)"></div>`,
        iconSize: [14, 14], iconAnchor: [7, 7], className: '',
      })
      L.marker([9.6412, -13.5784], { icon: userIcon }).addTo(map)
        ...
        L.marker([a.lat, a.lng], { icon }).addTo(map)
          .bindPopup(`
            <div style="min-width:160px;font-family:'Outfit',sans-serif">
              ...
              <p style="font-size:12px;margin:0;color:${a.available ? '#16A34A' : '#9CA3AF'}">
