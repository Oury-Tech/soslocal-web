...
import { getServicePhoto } from '@/lib/utils/service-photos'
...
export function ServiceCard({
  ...
  return (
    <Link
      ...
      className={cn(
        'group relative flex flex-col gap-3 rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] overflow-hidden transition-all duration-200 hover:border-brand-300 dark:hover:border-brand-700 hover:shadow-soft-lg hover:-translate-y-0.5',
        withPhoto ? '' : 'p-5',
        className,
      )}
    >
      {/* Photo (optionnelle) */}
      {withPhoto && (
        <div className="relative h-36 w-full overflow-hidden bg-[rgb(var(--muted))]">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={getServicePhoto(service.slug, service.name, 600)}
            alt={service.name}
            loading="lazy"
            className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105"
          />
          <span className="absolute bottom-2 left-2 h-10 w-10 rounded-xl bg-[rgb(var(--card))]/90 backdrop-blur text-brand-600 dark:text-brand-300 flex items-center justify-center shadow-soft">
            <ServiceIcon slug={service.slug} name={service.name} className="h-5 w-5" />
          </span>
        </div>
      )}
      ...
      <div className={cn(withPhoto && 'px-5')}>
      ...
      <div className={cn('mt-auto flex items-center gap-3 flex-wrap text-xs text-[rgb(var(--muted-fg))]', withPhoto && 'px-5')}>
      ...
      <div className={cn('flex items-end justify-between pt-2 border-t border-[rgb(var(--border))]', withPhoto && 'px-5 pb-5')}>
        <div>
          {from != null ? (
            <>
